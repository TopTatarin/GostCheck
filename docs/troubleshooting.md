...
Times New Roman declaration for formal inspection and falls back to Liberation
Serif at compile time via `\IfFontExistsTF`. If fallback lookup fails, verify
`fc-match "Liberation Serif"` and the `FreeSerif`, `FreeSans`, and `FreeMono`
families with `fc-match`, then install `fonts-liberation2` and
`fonts-freefont-ttf`.
